...
import { c as createLucideIcon, u as useQuery, a as useActor, b as createActor } from "./backend-CA2XpFJp.js";
import { S as Subscribable, s as shallowEqualObjects, h as hashKey, g as getDefaultState, n as notifyManager, e as useQueryClient, r as reactExports, f as noop$1, i as shouldThrowError, j as jsxRuntimeExports } from "./index-Di1Y5MzV.js";
...
function useDeleteTeamMember() {
  ...
  return useMutation({
    mutationFn: async (id2) => {
      if (!actor) throw new Error("Actor not available");
      return actor.deleteTeamMember(id2);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["teamMembers"] })
  });
}
...
function useDeleteActivity() {
  ...
  return useMutation({
    mutationFn: async (id2) => {
      if (!actor) throw new Error("Actor not available");
      return actor.deleteActivity(id2);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["activities"] })
  });
}
...
function useDeleteGalleryItem() {
  ...
  return useMutation({
    mutationFn: async (id2) => {
      if (!actor) throw new Error("Actor not available");
      return actor.deleteGalleryItem(id2);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["galleryItems"] })
  });
}
function useUpdateAdmissionInfo() {
  const { actor } = useBackendActor();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (info) => {
...
function useMarkContactRead() {
  ...
  return useMutation({
    mutationFn: async (id2) => {
      if (!actor) throw new Error("Actor not available");
      return actor.markContactRead(id2);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["contactSubmissions"] })
  });
}
export {
  useUpdateOrgInfo as A,
  useUpdateOrgLogo as B,
  ...
  useCamps as c,
  useBranches as d,
  useGalleryItems as e,
  MotionConfigContext as f,
  useConstant as g,
  usePresence as h,
  isHTMLElement as i,
  useIsomorphicLayoutEffect as j,
  useAdmissionInfo as k,
  useSubmitContact as l,
  motion as m,
  Mail as n,
  useUpdateAdmissionInfo as o,
  useContactSubmissions as p,
  useMarkContactRead as q,
  useAddGalleryItem as r,
  useDeleteGalleryItem as s,
  useAddBranch as t,
  useOrgInfo as u,
  useDeleteBranch as v,
  useAddActivity as w,
  useDeleteActivity as x,
  useAddTeamMember as y,
  useDeleteTeamMember as z
};
